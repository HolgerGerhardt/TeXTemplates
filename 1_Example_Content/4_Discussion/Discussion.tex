...
Let's reference some tables: \autoref{tab:basic_formatting} and \autoref{tab:advanced_formatting}. \Blindtext[3]
...
In deriving our predictions (\autoref{sec:Predictions}), we assume that subjects base their decisions on utility derived from receiving monetary payments $c_t$ at various dates~$t$. We also make the standard assumption that utility from money is increasing in its argument but not convex, i.e., ${u'(c_t) > 0}$ and ${u''(c_t) \le 0}$. Both assumptions are frequently made in studies on intertemporal decision making.
