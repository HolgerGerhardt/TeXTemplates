...
\renewcommand{\todo}[2][]{%
	\todoorig[#1, tickmarkheight=1.5pt]{#2}%
}
...
% Workaround for bug in the changes package (as of 2021-03-21, see https://gitlab.com/ekleinod/changes/-/issues/97): -->
\newsavebox{\addedbox}
\newsavebox{\replacedbox}
\newsavebox{\deletedbox}
\makeatletter
\savebox{\addedbox}{\@todonotes@useSizeCommand\textit{\color{white}Added text}}
\savebox{\replacedbox}{\@todonotes@useSizeCommand\textit{\color{white}Replaced:}}
\savebox{\deletedbox}{\@todonotes@useSizeCommand\textit{\color{white}Deleted:}}
\makeatother
% <--
...
\renewcommand{\added}[2][]{%
	\addedorig[#1]{#2}%
	\IfSubStr{#1}{comment}{}{\comment[#1]{\usebox{\addedbox}}}%
}
\LetLtxMacro{\replacedorig}{\replaced}
\renewcommand{\replaced}[3][]{%
	\replacedorig[#1, comment={\newline\usebox{\replacedbox} ``#3''}]{#2}{}%
}
\setdeletedmarkup{}
\LetLtxMacro{\deletedorig}{\deleted}
\renewcommand{\deleted}[2][]{%
	\deletedorig[#1]{#2}%
	\IfSubStr{#1}{comment}{}{\comment[#1]{\usebox{\deletedbox} ``#2''}}%
}
\sethighlightmarkup{%
	%\ifthenelse{\isColored}{\sethlcolor{authorcolor}}{}%
	\sethlcolor{authorcolor}%
	\hl{#1}%
}
\LetLtxMacro{\highlightorig}{\highlight}
\renewcommand{\highlight}[2][]{%
	\highlightorig[#1]{#2}%
	\IfSubStr{#1}{comment}{}{\comment[#1]{\\[-30pt]}}%
}
