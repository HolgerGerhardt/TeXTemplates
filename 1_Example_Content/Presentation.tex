...
\begin{frame}[allowframebreaks]{\titleprefix: Questions to Structure Your Presentation}
...
\begin{enumerate}
	...
	\item \alert{What is the research question? [R]}
...
\begin{enumerate}
	...
	\item[9.] \alert{How did we contribute to closing that gap, and what did we find? [A]} Which method\slash dataset did we use, and what are the results? \textcolor{gray}{[Recap and take-home message.]}
...
John Cochrane might do away with question~12.

See his ``Writing Tips for Ph.D. Students,'' \url{https://www.fma.org/assets/docs/membercontent/writing_cochrane.pdf}.

\end{frame}


\begin{frame}[allowframebreaks]{\titleprefix: More Structuring Advice}

In her book \textit{The Little Book of Research Writing}, Varanya Chaubey proposes a~simpler list of only three items---the \alert{``RAP method''}:
\begin{itemize}
	\item The \alert{``R''} stands for ``research question.''
	\item The \alert{``A''} stands for ``answer'' (to the research question, of course).
	\item The \alert{``P''} stands for ``positioning statement.'' (How does our manuscript relate to the literature: Does it corroborate or extend or contradict others' findings?)
\end{itemize}
...
For a quick summary of the book, see \url{https://mauve-porcupine-8992.squarespace.com/s/Chaubey_Research_Writing-bxdd.pdf}. See also \url{https://www.econscribe.org/about} and \url{https://arxiv.org/pdf/2012.07787}.
...
\NewTblrColumnType{E}{S[table-format = {\texteuro}2.2, table-align-text-before = true]}
...
\begin{table}
	...
	\resizebox*{!}{0.6\textheight}{{%
		\SetTblrInner[booktabs]{cells = {font = \footnotesize}}%
...
\NewTblrColumnType{U}{S[table-format = {**}+1.3{-**}, round-mode = places, round-precision = 3]}%
\providecommand{\discard}[1]{\makebox(0pt, 0pt)[lb]{}}
...
\begin{table}
	\caption{%
		...
		Make sure to mention the dependent variable (here, $m_\sim$)!%
	}
	\label{tab:lin_reg_interactions}
	\vspace{-\smallskipamount}%
	\resizebox*{!}{0.6\textheight}{%
		\SetTblrStyle{remark}{font = \scriptsize, halign = j, hang = 0pt, indent = 0pt}%
		\SetTblrInner[booktabs]{cells = {font = \footnotesize}}%
...
\SetTblrTemplate{remark}{empty}  % Suppress table notes in this frame
\SetTblrInner[booktabs]{
	cells = {font = \footnotesize},
	cell{2-Z}{2-Z} = {fg = white},
}
\only<2->{\SetTblrInner[booktabs]{column{2} = {fg = black}}}
\only<3->{\SetTblrInner[booktabs]{column{3} = {fg = black}}}
\only<4->{\SetTblrInner[booktabs]{column{4} = {fg = black}}}
\only<5->{\SetTblrInner[booktabs]{column{5} = {fg = black}}}
\only<6>{\SetTblrInner[booktabs]{column{6} = {fg = black}}}
...
\only<1>{\SetTblrInner[booktabs]{cell{2}{2} = {bg = UBonnYellow!75, fg = black}}}
\only<2>{\SetTblrInner[booktabs]{row{2} = {bg = UBonnYellow!75, fg = black}}}
\only<3>{\SetTblrInner[booktabs]{column{4} = {bg = UBonnYellow!75, fg = black}}}
\only<4>{%
	\SetTblrInner[booktabs]{
		cell{12}{2, 3, 5, 6} = {bg = Chartreuse4, fg = white, font = \bfseries},
		cell{12}{4} = {bg = Red3, fg = white, font = \bfseries},
		column{1-5} = {rightsep+ = -0.36em},
		column{2-6} = {leftsep+ = -0.36em},
	}%
}
...
\begin{table}
	\caption{%
		\textit{tabularray} makes it simple to highlight cells, rows, or columns in a~table.%
	}
	\vspace{-\smallskipamount}%
	\resizebox*{!}{0.67\textheight}{%
		\input{1_Example_Content/9_Appendix/siunitx_Example_Regression}%
...
\begin{itemize}[<+->]
	...
	\item The latex is therefore usually allowed to coagulate on the tree \citep{Koszegi2013}.
		...
		\end{itemize}
	\item See also \cite{Bursztyn2025, Bordalo2013, Dohmen2012}.
\end{itemize}
