% !TeX program = pdflatex
% !BIB program = biber



\section{Results}
\label{sec:Results}

\blindtext
With this, we can test our hypotheses.

\makeatletter
\@ifclassloaded{book}{
	\subsection{%
		Test of
		\texorpdfstring{\autoref{hy:UNBAL}}{\autoref{\currentchapter:hy:UNBAL}}%
	}
}{
	\subsection{Test of \autoref{hy:UNBAL}}
}
\makeatother

Our first result supports \autoref{hy:UNBAL}. \blindtext The analysis we conducted to obtain \autoref{r:main} is described in detail in \autoref{tmain:focus}. Let's reference a~section, a~subsection, and a~figure from the appendices: \autoref{sec:app:example_tables}, \autoref{sec:app:salience}, \autoref{fig:concentrated_cl}.

\begin{result} \label{r:main}
	\blindtext
\end{result}

\begin{table}[t]
	\caption{A~\textit{tabularray}-Based Example Table}
	\label{tmain:focus}
	\begin{booktabs}[
		%caption = {A~\textit{tabularray}-Based Example Table},  % Does not work with the automated chapter-wise labels for tables implemented in the dissertation template
		entry = none,
		footsep = \belowbottomsep,
		headsep = 0pt,
		%label = {tmain:focus},  % Does not work with the automated chapter-wise labels for tables implemented in the dissertation template
		label = none,
		remark{Notes} = {Standard errors in parentheses, clustered on the subject level. {*} $p < 0.10$, {**} $p < 0.05$, {***} $p<0.01$.},
		tall,
	]{
		colspec = {X[l] T{1.3}},
		width = \textwidth,
		hborder{4} = {abovespace = \aboverulesep, belowspace = \belowrulesep},
		hline{4} = {\lightrulewidth, solid},
	}
		Dependent variable & {$\hat{d}$} \\
		Estimate      &       0.123{***} \\
		&     (0.011) \\
		Observations & {750} \\
		Subjects      &         {250} \\
	\end{booktabs}
\end{table}

\blindtext

\makeatletter
\@ifclassloaded{book}{
	\subsection{%
		Test of
		\texorpdfstring{\autoref{hy:UNBAL_2}}{\autoref{\currentchapter:hy:UNBAL_2}}%
	}
}{
	\subsection{Test of \autoref{hy:UNBAL_2}}
}
\makeatother

\blindtext We thereby test \autoref{hy:UNBAL_2}.

\begin{result} \label{r:main2}
	\blindtext
\end{result}

Our second result provides evidence in support of \autoref{hy:UNBAL_2}.\blindmathfalse\blindtext\blindmathtrue

\subsection{Heterogeneity}
\blindmathpaper