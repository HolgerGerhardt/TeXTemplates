...
\addbibresource{biblatex-examples-ms.bib}
...
\newcommand{\currentchapter}{ChIntro}
...
% Extend \renewcommand{\label}{...} to figure environment
\let \origfigure \figure
\let \origendfigure \endfigure
\renewenvironment{figure}[1][]{%
	\origfigure[#1]%
	\renewcommand{\label}[1]{%
		\labelorig{\currentchapter:##1}%
	}%
}{\origendfigure}
% Extend \renewcommand{\label}{...} to table environment
\let \origtable \table
...
\renewcommand{\currentchapter}{Ch1}

\begin{refsection}

% Revert to original footnote text definition for symobl-based footnote
\makeatletter
...
% Switch back to numbered footnotes
\renewcommand{\thefootnote}{\thefootnoteorig}
% Switch back to custom footnote text definition
\makeatletter
...
\begin{abstract}
	% The input file must not contain blank lines before the abstract text, as these are interpreted as the start of a new paragraph.
...
\makeatother
\setcounter{footnote}{0}
\textsf{\Large\textit{Joint with Adam Smith, Janet Smith, and Jeremiah Smith}}
...
\iftrue  % Use \iffalse to omit the math tests
